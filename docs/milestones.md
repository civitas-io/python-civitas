...
| v0.9.3.8 | **Live tick animation** for `CostChart`/`MessageRateChart` — ❌ **Investigated, declined (v0.10.1)**: a terminal `plotext` chart redraws with real data each poll; "animating between ticks" would mean interpolating *fake* cost/rate frames between real polls — dishonest data on a cost dashboard. No meaningful animation to build; the per-poll redraw IS the update | Declined |
